...
public class Monster implements PlayerInterface {
	...
	private int iBehaveStatus = InfiniteCst.NPC_BEHAVE_TALK;
	 ...
	 */
	private int currLifePoint = 0;
	private int currMagicPoint = 0;
	private int currActionPoint = 0;
	private int currCharmPoint = 0;


	public Monster(String name, DaoManager manager){

		setDaoManager(manager);
		npc = getDaoManager().getNpcByName(name);
...
*/
		...
		case InfiniteCst.NPC_BEHAVE_FIGHT:
			if(getSpellBookFight().size()>0){
				...
				int ichoose = GenericUtil.getRandomNumber(0, (getSpellBookFight().size()-1) ); 
				addToPreparedSpells( getSpellBookFight().get(ichoose) );

				//if cannot cast chosen spell revert to melee
		...
		case InfiniteCst.NPC_BEHAVE_HEAL:
			if(getSpellBookHeal().size()>0){
				//TODO AI must choose best spell
				int ichoose =  GenericUtil.getRandomNumber(0, (getSpellBookHeal().size()-1) ); 
		...
		case InfiniteCst.NPC_BEHAVE_DEFEND:
			if(getSpellBookProtect().size()>0 && getPointsMagic()>0 ){
				//TODO AI must choose best spell
				int ichoose =  GenericUtil.getRandomNumber(0, (getSpellBookProtect().size()-1) ); 
		...
		if(iNumTypes==0 && getPointsAction()<=0){
			...
			iAttackKind =  GenericUtil.getRandomNumber(0, iNumTypes  );
		}
...
public String[] getMeleeAttacks( int round){
	...
	public void learnSpells(String[] spellsNames){
		...
		//fighting spells
		List<Spell> l = getDaoManager().getSpellsByNameAndType(spellsNames, InfiniteCst.MAGIC_ATTACK );
		for (int i = 0; i < l.size(); i++) {
			spellBookFight.add( new PlayerKnowSpell(null,l.get(i),0) );
		}		

		//healing spells
		l = getDaoManager().getSpellsByNameAndType(spellsNames, InfiniteCst.MAGIC_HEAL );
		for (int i = 0; i < l.size(); i++) {
			spellBookHeal.add( new PlayerKnowSpell(null,l.get(i),0) );
		}

		//protection spells
		l = getDaoManager().getSpellsByNameAndType(spellsNames, InfiniteCst.MAGIC_DEFEND );
		for (int i = 0; i < l.size(); i++) {
	...
	public void equipItems(String[] itemsNames){
		...
		List<Item> l = getDaoManager().getItemsByName(itemsNames);
		for (int i = 0; i < l.size(); i++) {
